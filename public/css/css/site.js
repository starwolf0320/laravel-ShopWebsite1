...
!function (a, b) {
    ...
    "object" == typeof exports && exports ? factory(exports) : "function" == typeof define && define.amd ? define(["exports"], factory) : factory(global.Mustache = {})
    ...
    function parseTemplate(template, tags) {
        ...
        function compileTags(tags) {
            if ("string" == typeof tags && (tags = tags.split(spaceRe, 2)), !isArray(tags) || 2 !== tags.length)throw new Error("Invalid tags: " + tags);
        ...
        for (var start, type, value, chr, token, openSection, scanner = new Scanner(template); !scanner.eos();) {
            if (start = scanner.pos, value = scanner.scanUntil(openingTagRe))for (var i = 0, valueLength = value.length; i < valueLength; ++i)chr = value.charAt(i), isWhitespace(chr) ? spaces.push(tokens.length) : nonSpace = !0, tokens.push(["text", chr, start, start + 1]), start += 1, "\n" === chr && stripSpace();
    ...
    function Context(view, parentContext) {
        this.view = null == view ? {} : view, this.cache = {".": this.view}, this.parent = parentContext
    }
    ...
    var Object_toString = Object.prototype.toString, isArray = Array.isArray || function (object) {
        ...
        }, RegExp_test = RegExp.prototype.test, nonSpaceRe = /\S/, entityMap = {
        "&": "&amp;",
        "<": "&lt;",
        ">": "&gt;",
        '"': "&quot;",
    ...
    Scanner.prototype.eos = function () {
        ...
        if (name in cache)value = cache[name]; else {
            for (var names, index, context = this; context;) {
                if (name.indexOf(".") > 0)for (value = context.view, names = name.split("."), index = 0; null != value && index < names.length;)value = value[names[index++]]; else value = context.view[name];
                if (null != value)break;
        ...
        for (var token, value, buffer = "", self = this, i = 0, numTokens = tokens.length; i < numTokens; ++i)switch (token = tokens[i], token[0]) {
            case"#":
                if (value = context.lookup(token[1]), !value)continue;
                if (isArray(value))for (var j = 0, valueLength = value.length; j < valueLength; ++j)buffer += this.renderTokens(token[4], context.push(value[j]), partials, originalTemplate); else if ("object" == typeof value || "string" == typeof value)buffer += this.renderTokens(token[4], context.push(value), partials, originalTemplate); else if (isFunction(value)) {
    ...
    function barPositionCSS(n, speed, ease) {
        var barCSS;
        return barCSS = "translate3d" === Settings.positionUsing ? {transform: "translate3d(" + toBarPerc(n) + "%,0,0)"} : "translate" === Settings.positionUsing ? {transform: "translate(" + toBarPerc(n) + "%,0)"} : {"margin-left": toBarPerc(n) + "%"}, barCSS.transition = "all " + speed + "ms " + ease, barCSS
    }
    ...
    NProgress.configure = function (options) {
        ...
        return progress.offsetWidth, queue(function (next) {
            "" === Settings.positionUsing && (Settings.positionUsing = NProgress.getPositioningCSS()), css(bar, barPositionCSS(n, speed, ease)), 1 === n ? (css(progress, {
                ...
                css(progress, {transition: "all " + speed + "ms linear", opacity: 0}), setTimeout(function () {
    ...
    var $buttons = CSbuttons.cache.$shareButtons, $shareLinks = $buttons.find("a");
    $buttons.attr("data-permalink"), $(".share-facebook"), $(".share-twitter"), $(".share-pinterest"), $(".share-google");
    $shareLinks.on("click", function (e) {
        e.preventDefault();
        var el = $(this), popup = el.attr("class").replace("-", "_"), link = el.attr("href"), w = 700, h = 400;
        switch (popup) {
            case"share-twitter":
                h = 300;
                break;
    ...
    "use strict";
    var _currentElement, _copyTarget, _window = window, _document = _window.document, _navigator = _window.navigator, _setTimeout = _window.setTimeout, _encodeURIComponent = _window.encodeURIComponent, _ActiveXObject = _window.ActiveXObject, _Error = _window.Error, _parseInt = _window.Number.parseInt || _window.parseInt, _parseFloat = _window.Number.parseFloat || _window.parseFloat, _isNaN = _window.Number.isNaN || _window.isNaN, _round = _window.Math.round, _now = _window.Date.now, _keys = _window.Object.keys, _defineProperty = _window.Object.defineProperty, _hasOwn = _window.Object.prototype.hasOwnProperty, _slice = _window.Array.prototype.slice, _unwrap = function () {
        ...
        for (i = 1, len = args.length; i < len; i++)if (null != (arg = args[i]))for (prop in arg)_hasOwn.call(arg, prop) && (src = target[prop], copy = arg[prop], target !== copy && copy !== undefined && (target[prop] = copy));
        return target
    }, _deepCopy = function (source) {
        var copy, i, len, prop;
        if ("object" != typeof source || null == source)copy = source; else if ("number" == typeof source.length)for (copy = [], i = 0, len = source.length; i < len; i++)_hasOwn.call(source, i) && (copy[i] = _deepCopy(source[i])); else {
            copy = {};
            for (prop in source)_hasOwn.call(source, prop) && (copy[prop] = _deepCopy(source[prop]))
        }
        return copy
    }, _pick = function (obj, keys) {
        for (var newObj = {}, i = 0, len = keys.length; i < len; i++)keys[i] in obj && (newObj[keys[i]] = obj[keys[i]]);
        return newObj
    }, _omit = function (obj, keys) {
        var newObj = {};
        for (var prop in obj)keys.indexOf(prop) === -1 && (newObj[prop] = obj[prop]);
        return newObj
        ...
        var dir;
        return "string" == typeof url && url && (dir = url.split("#")[0].split("?")[0], dir = url.slice(0, url.lastIndexOf("/") + 1)), dir
    }, _getCurrentScriptUrlFromErrorStack = function (stack) {
        var url, matches;
        return "string" == typeof stack && stack && (matches = stack.match(/^(?:|[^:@]*@|.+\)@(?=http[s]?|file)|.+?\s+(?: at |@)(?:[^:\(]+ )*[\(]?)((?:http[s]?|file):\/\/[\/]?.+?\/[^:\)]*?)(?::\d+)(?::\d+)?/), matches && matches[1] ? url = matches[1] : (matches = stack.match(/\)@((?:http[s]?|file):\/\/[\/]?.+?\/[^:\)]*?)(?::\d+)(?::\d+)?/), matches && matches[1] && (url = matches[1]))), url
        ...
        return err && (url = err.sourceURL || err.fileName || _getCurrentScriptUrlFromErrorStack(err.stack)), url
        ...
        overdue: null,
        ready: null
    }, _minimumFlashVersion = "11.0.0", _handlers = {}, _clipData = {}, _clipDataFormatMap = null, _eventMessages = {
        ...
        return {
            ...
            zeroclipboard: {version: ZeroClipboard.version, config: ZeroClipboard.config()}
        }
        ...
        if (events && events.length) {
            for (i = 0, len = events.length; i < len; i++)eventType = events[i].replace(/^on/, ""), added[eventType] = !0, _handlers[eventType] || (_handlers[eventType] = []), _handlers[eventType].push(listener);
            if (added.ready && _flashState.ready && ZeroClipboard.emit({type: "ready"}), added.error) {
                var errorTypes = ["disabled", "outdated", "unavailable", "deactivated", "overdue"];
                for (i = 0, len = errorTypes.length; i < len; i++)if (_flashState[errorTypes[i]] === !0) {
                    ZeroClipboard.emit({type: "error", name: "flash-" + errorTypes[i]});
                    break
                }
            }
        }
        ...
        if (events && events.length)for (i = 0, len = events.length; i < len; i++)if (eventType = events[i].toLowerCase().replace(/^on/, ""), perEventHandlers = _handlers[eventType], perEventHandlers && perEventHandlers.length)if (listener)for (foundIndex = perEventHandlers.indexOf(listener); foundIndex !== -1;)perEventHandlers.splice(foundIndex, 1), foundIndex = perEventHandlers.indexOf(listener, foundIndex); else perEventHandlers.length = 0;
        return ZeroClipboard
        ...
        if (event = _createEvent(event), event && !_preprocessEvent(event))return "ready" === event.type && _flashState.overdue === !0 ? ZeroClipboard.emit({
            type: "error",
            name: "flash-overdue"
        }) : (eventCopy = _extend({}, event), _dispatchCallbacks.call(this, eventCopy), "copy" === event.type && (tmp = _mapClipDataToFlash(_clipData), returnVal = tmp.data, _clipDataFormatMap = tmp.formatMap), returnVal)
    }, _create = function () {
        if ("boolean" != typeof _flashState.ready && (_flashState.ready = !1), !ZeroClipboard.isFlashUnusable() && null === _flashState.bridge) {
            var maxWait = _globalConfig.flashLoadTimeout;
            "number" == typeof maxWait && maxWait >= 0 && _setTimeout(function () {
        ...
        ZeroClipboard.clearData(), ZeroClipboard.blur(), ZeroClipboard.emit("destroy"), _unembedSwf(), ZeroClipboard.off()
    }, _setData = function (format, data) {
        var dataObj;
        if ("object" == typeof format && format && "undefined" == typeof data)dataObj = format, ZeroClipboard.clearData(); else {
        ...
        if (element && 1 === element.nodeType) {
            _currentElement && (_removeClass(_currentElement, _globalConfig.activeClass), _currentElement !== element && _removeClass(_currentElement, _globalConfig.hoverClass)), _currentElement = element, _addClass(element, _globalConfig.hoverClass);
        ...
        var htmlBridge = _getHtmlBridge(_flashState.bridge);
        htmlBridge && (htmlBridge.removeAttribute("title"), htmlBridge.style.left = "0px", htmlBridge.style.top = "-9999px", htmlBridge.style.width = "1px", htmlBridge.style.top = "1px"), _currentElement && (_removeClass(_currentElement, _globalConfig.hoverClass), _removeClass(_currentElement, _globalConfig.activeClass), _currentElement = null)
        ...
        var eventType;
        if ("string" == typeof event && event ? (eventType = event, event = {}) : "object" == typeof event && event && "string" == typeof event.type && event.type && (eventType = event.type), eventType) {
            !event.target && /^(copy|aftercopy|_click)$/.test(eventType.toLowerCase()) && (event.target = _copyTarget), _extend(event, {
            ...
            return "error" === event.type && event.name && msg && (msg = msg[event.name]), msg && (event.message = msg), "ready" === event.type && _extend(event, {
                target: null,
                version: _flashState.version
            }), "error" === event.type && (/^flash-(disabled|outdated|unavailable|deactivated|overdue)$/.test(event.name) && _extend(event, {
                target: null,
                minimumVersion: _minimumFlashVersion
            }), /^flash-(outdated|unavailable|deactivated|overdue)$/.test(event.name) && _extend(event, {version: _flashState.version})), "copy" === event.type && (event.clipboardData = {
                setData: ZeroClipboard.setData,
                clearData: ZeroClipboard.clearData
            }), "aftercopy" === event.type && (event = _mapClipResultsFromFlash(event, _clipDataFormatMap)), event.target && !event.relatedTarget && (event.relatedTarget = _getRelatedTarget(event.target)), event = _addMouseData(event)
        }
        ...
        if ("object" == typeof event && event && event.type) {
            ...
            if (handlers && handlers.length) {
                ...
                for (i = 0, len = handlers.length; i < len; i++)func = handlers[i], context = originalContext, "string" == typeof func && "function" == typeof _window[func] && (func = _window[func]), "object" == typeof func && func && "function" == typeof func.handleEvent && (context = func, func = func.handleEvent), "function" == typeof func && (eventCopy = _extend({}, event), _dispatchCallback(func, context, [eventCopy], async))
            }
            return this
        }
        ...
        switch (event.type) {
            case"error":
                flashErrorNames.indexOf(event.name) !== -1 && _extend(_flashState, {
            ...
            case"copy":
                ...
                break;
            case"aftercopy":
                ZeroClipboard.clearData(), element && element !== _safeActiveElement() && element.focus && element.focus();
                break;
            case"_mouseover":
            ...
            case"_mouseout":
                ...
                break;
            case"_mousedown":
                _addClass(element, _globalConfig.activeClass), _globalConfig.bubbleEvents === !0 && sourceIsSwf && _fireMouseEvent(_extend({}, event, {type: event.type.slice(1)}));
                break;
            case"_mouseup":
                _removeClass(element, _globalConfig.activeClass), _globalConfig.bubbleEvents === !0 && sourceIsSwf && _fireMouseEvent(_extend({}, event, {type: event.type.slice(1)}));
                break;
            case"_click":
                _copyTarget = null, _globalConfig.bubbleEvents === !0 && sourceIsSwf && _fireMouseEvent(_extend({}, event, {type: event.type.slice(1)}));
                break;
            case"_mousemove":
                _globalConfig.bubbleEvents === !0 && sourceIsSwf && _fireMouseEvent(_extend({}, event, {type: event.type.slice(1)}))
        }
        ...
        if (event && "string" == typeof event.type && event) {
            ...
            target && doc.createEvent && target.dispatchEvent && (args = [args.type, args.canBubble, args.cancelable, args.view, args.detail, args.screenX, args.screenY, args.clientX, args.clientY, args.ctrlKey, args.altKey, args.shiftKey, args.metaKey, args.button, args.relatedTarget], e = doc.createEvent("MouseEvents"), e.initMouseEvent && (e.initMouseEvent.apply(e, args), e._source = "js", target.dispatchEvent(e)))
        }
    }, _createHtmlBridge = function () {
        var container = _document.createElement("div");
        return container.id = _globalConfig.containerId, container.className = _globalConfig.containerClass, container.style.position = "absolute", container.style.left = "0px", container.style.top = "-9999px", container.style.width = "1px", container.style.height = "1px", container.style.zIndex = "" + _getSafeZIndex(_globalConfig.zIndex), container
        ...
        if (!flashBridge) {
            var allowScriptAccess = _determineScriptAccess(_window.location.host, _globalConfig), allowNetworking = "never" === allowScriptAccess ? "none" : "all", flashvars = _vars(_globalConfig), swfUrl = _globalConfig.swfPath + _cacheBust(_globalConfig.swfPath, _globalConfig);
            container = _createHtmlBridge();
            var divToBeReplaced = _document.createElement("div");
            container.appendChild(divToBeReplaced), _document.body.appendChild(container);
            var tmpDiv = _document.createElement("div"), oldIE = "activex" === _flashState.pluginType;
        ...
        return flashBridge || (flashBridge = _document[_globalConfig.swfObjectId], flashBridge && (len = flashBridge.length) && (flashBridge = flashBridge[len - 1]), !flashBridge && container && (flashBridge = container.firstChild)), _flashState.bridge = flashBridge || null, flashBridge
    }, _unembedSwf = function () {
        var flashBridge = _flashState.bridge;
        if (flashBridge) {
            var htmlBridge = _getHtmlBridge(flashBridge);
            htmlBridge && ("activex" === _flashState.pluginType && "readyState" in flashBridge ? (flashBridge.style.display = "none", function removeSwfFromIE() {
            ...
            }()) : (flashBridge.parentNode && flashBridge.parentNode.removeChild(flashBridge), htmlBridge.parentNode && htmlBridge.parentNode.removeChild(htmlBridge))), _flashState.ready = null, _flashState.bridge = null, _flashState.deactivated = null
        }
        ...
        var newResults = {};
        for (var prop in clipResults)if (_hasOwn.call(clipResults, prop)) {
            if ("success" !== prop && "data" !== prop) {
        ...
        return cacheBust ? (path.indexOf("?") === -1 ? "?" : "&") + "noCache=" + _now() : ""
    }, _vars = function (options) {
        var i, len, domain, domains, str = "", trustedOriginsExpanded = [];
        if (options.trustedDomains && ("string" == typeof options.trustedDomains ? domains = [options.trustedDomains] : "object" == typeof options.trustedDomains && "length" in options.trustedDomains && (domains = options.trustedDomains)), domains && domains.length)for (i = 0, len = domains.length; i < len; i++)if (_hasOwn.call(domains, i) && domains[i] && "string" == typeof domains[i]) {
        ...
        return trustedOriginsExpanded.length && (str += "trustedOrigins=" + _encodeURIComponent(trustedOriginsExpanded.join(","))), options.forceEnhancedClipboard === !0 && (str += (str ? "&" : "") + "forceEnhancedClipboard=true"), "string" == typeof options.swfObjectId && options.swfObjectId && (str += (str ? "&" : "") + "swfObjectId=" + _encodeURIComponent(options.swfObjectId)), str
        ...
        return originOrUrl = pathIndex === -1 ? originOrUrl : protocolIndex === -1 || 0 === pathIndex ? null : originOrUrl.slice(0, pathIndex), originOrUrl && ".swf" === originOrUrl.slice(-4).toLowerCase() ? null : originOrUrl || null
        ...
        var _extractAllDomains = function (origins) {
            ...
            for (i = 0, len = origins.length; i < len; i++)if (_hasOwn.call(origins, i) && (tmp = _extractDomain(origins[i]))) {
                if ("*" === tmp) {
                    resultsArray.length = 0, resultsArray.push("*");
                    break
                }
                resultsArray.indexOf(tmp) === -1 && resultsArray.push(tmp)
            }
            return resultsArray
        };
        return function (currentDomain, configOptions) {
            ...
            if (len > 0) {
                ...
                if (trustedDomains.indexOf(currentDomain) !== -1)return 1 === len && currentDomain === swfDomain ? "sameDomain" : "always"
            }
            return "never"
        }
        ...
        if (element.classList)return element.classList.contains(value) || element.classList.add(value), element;
        if (value && "string" == typeof value) {
            var classNames = (value || "").split(/\s+/);
            if (1 === element.nodeType)if (element.className) {
                for (var className = " " + element.className + " ", setClass = element.className, c = 0, cl = classNames.length; c < cl; c++)className.indexOf(" " + classNames[c] + " ") < 0 && (setClass += " " + classNames[c]);
        ...
        if (!element || 1 !== element.nodeType)return element;
        if (element.classList)return element.classList.contains(value) && element.classList.remove(value), element;
        if ("string" == typeof value && value) {
            var classNames = value.split(/\s+/);
            if (1 === element.nodeType && element.className) {
                for (var className = (" " + element.className + " ").replace(/[\n\t]/g, " "), c = 0, cl = classNames.length; c < cl; c++)className = className.replace(" " + classNames[c] + " ", " ");
        ...
        return "function" == typeof _document.body.getBoundingClientRect && (rect = _document.body.getBoundingClientRect(), physicalWidth = rect.right - rect.left, logicalWidth = _document.body.offsetWidth, zoomFactor = _round(physicalWidth / logicalWidth * 100) / 100), zoomFactor
        ...
        if (obj.getBoundingClientRect) {
            ...
            "pageXOffset" in _window && "pageYOffset" in _window ? (pageXOffset = _window.pageXOffset, pageYOffset = _window.pageYOffset) : (zoomFactor = _getZoomFactor(), pageXOffset = _round(_document.documentElement.scrollLeft / zoomFactor), pageYOffset = _round(_document.documentElement.scrollTop / zoomFactor));
            var leftBorderWidth = _document.documentElement.clientLeft || 0, topBorderWidth = _document.documentElement.clientTop || 0;
            info.left = rect.left + pageXOffset - leftBorderWidth, info.top = rect.top + pageYOffset - topBorderWidth, info.width = "width" in rect ? rect.width : rect.right - rect.left, info.height = "height" in rect ? rect.height : rect.bottom - rect.top
        }
        return info
        ...
        var zIndex;
        return "number" != typeof val || _isNaN(val) ? "string" == typeof val && (zIndex = _getSafeZIndex(_parseInt(val, 10))) : zIndex = val, "number" == typeof zIndex ? zIndex : "auto"
        ...
        function inspectPlugin(plugin) {
            plugin && (hasFlash = !0, plugin.version && (flashVersion = parseFlashVersion(plugin.version)), !flashVersion && plugin.description && (flashVersion = parseFlashVersion(plugin.description)), plugin.filename && (isPPAPI = isPepperFlash(plugin.filename)))
        }

        var plugin, ax, mimeType, hasFlash = !1, isActiveX = !1, isPPAPI = !1, flashVersion = "";
        if (_navigator.plugins && _navigator.plugins.length)plugin = _navigator.plugins["Shockwave Flash"], inspectPlugin(plugin), _navigator.plugins["Shockwave Flash 2.0"] && (hasFlash = !0, flashVersion = "2.0.0.11"); else if (_navigator.mimeTypes && _navigator.mimeTypes.length)mimeType = _navigator.mimeTypes["application/x-shockwave-flash"],
        ...
        _flashState.disabled = hasFlash !== !0, _flashState.outdated = flashVersion && _parseFloat(flashVersion) < _parseFloat(_minimumFlashVersion), _flashState.version = flashVersion || "0.0.0", _flashState.pluginType = isPPAPI ? "pepper" : isActiveX ? "activex" : hasFlash ? "netscape" : "unknown"
    };
    ...
    _extend(_globalConfig, {autoActivate: !0});
    var _clientConstructor = function (elements) {
        ...
        if (events && events.length) {
            for (i = 0, len = events.length; i < len; i++)eventType = events[i].replace(/^on/, ""), added[eventType] = !0, handlers[eventType] || (handlers[eventType] = []), handlers[eventType].push(listener);
            if (added.ready && _flashState.ready && this.emit({type: "ready", client: this}), added.error) {
                var errorTypes = ["disabled", "outdated", "unavailable", "deactivated", "overdue"];
                for (i = 0, len = errorTypes.length; i < len; i++)if (_flashState[errorTypes[i]]) {
                    this.emit({type: "error", name: "flash-" + errorTypes[i], client: this});
                    break
                }
            }
        }
        ...
        if (events && events.length)for (i = 0, len = events.length; i < len; i++)if (eventType = events[i].toLowerCase().replace(/^on/, ""), perEventHandlers = handlers[eventType], perEventHandlers && perEventHandlers.length)if (listener)for (foundIndex = perEventHandlers.indexOf(listener); foundIndex !== -1;)perEventHandlers.splice(foundIndex, 1), foundIndex = perEventHandlers.indexOf(listener, foundIndex); else perEventHandlers.length = 0;
        return this
    }, _clientListeners = function (eventType) {
        var copy = null, handlers = _clientMeta[this.id] && _clientMeta[this.id].handlers;
        return handlers && (copy = "string" == typeof eventType && eventType ? handlers[eventType] ? handlers[eventType].slice(0) : [] : _deepCopy(handlers)), copy
        ...
        for (var i = 0; i < elements.length; i++)if (_hasOwn.call(elements, i) && elements[i] && 1 === elements[i].nodeType) {
            elements[i].zcClippingId ? _elementMeta[elements[i].zcClippingId].indexOf(this.id) === -1 && _elementMeta[elements[i].zcClippingId].push(this.id) : (elements[i].zcClippingId = "zcClippingId_" + _elementIdCounter++, _elementMeta[elements[i].zcClippingId] = [this.id], _globalConfig.autoActivate === !0 && _addMouseHandlers(elements[i]));
            var clippedElements = _clientMeta[this.id] && _clientMeta[this.id].elements;
            clippedElements.indexOf(elements[i]) === -1 && clippedElements.push(elements[i])
        }
        return this
        ...
        for (var i = elements.length; i--;)if (_hasOwn.call(elements, i) && elements[i] && 1 === elements[i].nodeType) {
            for (arrayIndex = 0; (arrayIndex = clippedElements.indexOf(elements[i], arrayIndex)) !== -1;)clippedElements.splice(arrayIndex, 1);
            var clientIds = _elementMeta[elements[i].zcClippingId];
            if (clientIds) {
                for (arrayIndex = 0; (arrayIndex = clientIds.indexOf(this.id, arrayIndex)) !== -1;)clientIds.splice(arrayIndex, 1);
                0 === clientIds.length && (_globalConfig.autoActivate === !0 && _removeMouseHandlers(elements[i]), delete elements[i].zcClippingId)
            }
        }
        return this
        ...
        var clippedEls = _clientMeta[this.id] && _clientMeta[this.id].elements, hasClippedEls = !!clippedEls && clippedEls.length > 0, goodTarget = !event.target || hasClippedEls && clippedEls.indexOf(event.target) !== -1, goodRelTarget = event.relatedTarget && hasClippedEls && clippedEls.indexOf(event.relatedTarget) !== -1, goodClient = event.client && event.client === this;
        return !!(goodTarget || goodRelTarget || goodClient)
    }, _clientDispatchCallbacks = function (event) {
        if ("object" == typeof event && event && event.type) {
            var async = _shouldPerformAsync(event), wildcardTypeHandlers = _clientMeta[this.id] && _clientMeta[this.id].handlers["*"] || [], specificTypeHandlers = _clientMeta[this.id] && _clientMeta[this.id].handlers[event.type] || [], handlers = wildcardTypeHandlers.concat(specificTypeHandlers);
            if (handlers && handlers.length) {
                var i, len, func, context, eventCopy, originalContext = this;
                for (i = 0, len = handlers.length; i < len; i++)func = handlers[i], context = originalContext, "string" == typeof func && "function" == typeof _window[func] && (func = _window[func]), "object" == typeof func && func && "function" == typeof func.handleEvent && (context = func, func = func.handleEvent), "function" == typeof func && (eventCopy = _extend({}, event), _dispatchCallback(func, context, [eventCopy], async))
            }
        ...
        if (element && 1 === element.nodeType) {
            var _suppressMouseEvents = function (event) {
                (event || (event = _window.event)) && ("js" !== event._source && (event.stopImmediatePropagation(), event.preventDefault()), delete event._source)
            }, _elementMouseOver = function (event) {
                (event || (event = _window.event)) && (_suppressMouseEvents(event), ZeroClipboard.focus(element))
            };
            element.addEventListener("mouseover", _elementMouseOver, !1), element.addEventListener("mouseout", _suppressMouseEvents, !1), element.addEventListener("mouseenter", _suppressMouseEvents, !1), element.addEventListener("mouseleave", _suppressMouseEvents, !1), element.addEventListener("mousemove", _suppressMouseEvents, !1), _mouseHandlers[element.zcClippingId] = {
        ...
        if (element && 1 === element.nodeType) {
            ...
            if ("object" == typeof mouseHandlers && mouseHandlers) {
                for (var key, val, mouseEvents = ["move", "leave", "enter", "out", "over"], i = 0, len = mouseEvents.length; i < len; i++)key = "mouse" + mouseEvents[i], val = mouseHandlers[key], "function" == typeof val && element.removeEventListener(key, val, !1);
    ...
    $.fn.getContrastingText = function (hex) {
        ...
        return threshold = 180, hRed = hexToR(hex), hGreen = hexToG(hex), hBlue = hexToB(hex), cBrightness = (299 * hRed + 587 * hGreen + 114 * hBlue) / 1e3, cBrightness > threshold ? "#313131" : "#ffffff"
    }
}(jQuery), function ($) {
    $.fn.cssMap = function (map) {
        var $element = this;
        return $.each(map, function (property, value) {
            if (value instanceof Array)for (var i = 0, len = value.length; i < len; i++)$element.css(property, value[i]); else $element.css(property, value)
        }), this
    }
}(jQuery), window.uiGradients = window.uiGradients || {}, function (window, document, $) {
    "use strict";
    uiGradients.Codeblock = function () {
        function _resetCodebox() {
            codebox.show(), success.hide()
        }

        function _updateCodebox() {
            codebox.hide(), success.slideDown()
        }

        function _setup() {
            var client = new ZeroClipboard($("#do-copy-to-clipboard"), {moviePath: "ZeroClipboard.swf", debug: !1});
        ...
        var canvas = $("#canvas"), codebox = $(".popup__codeblock"), success = $(".popup__success");
        return {init: init}
    }(), window.uiGradients.Codeblock.init()
}(window, document, $), window.uiGradients = window.uiGradients || {}, function (window, document, $) {
    "use strict";
    uiGradients.Gradients = function () {
        function _loadGradients() {
            $.ajax({
                url: "gradients.json", dataType: "json", beforeSend: function () {
                    NProgress.start()
                }, success: function (data) {
                    gradients = data, canvas.trigger("gradientsLoaded")
                }, complete: function () {
                    NProgress.done(), _transitionInSite()
                }
            })
        }
        ...
        function _transitionInSite() {
            setTimeout(function () {
                $("#js-preload").fadeOut(), $("#js-header, #js-main").addClass("is-active")
            }, 1e3)
        }

        function _updateGradient() {
            var newGradient = gradients[gradientIndex], from = newGradient.colors[0], to = newGradient.colors[1], gradientStr = "to left, " + from + "," + to, gradientObject = {
                "background-color": to,
                "background-image": ["-webkit-linear-gradient(" + gradientStr + ")", "linear-gradient(" + gradientStr + ")"]
            };
            canvas.stop().animate({opacity: .5}, 100, function () {
        ...
        function _loadHash() {
            ...
            $.each(gradients, function (index, item) {
                if (item.name.replace(/\s/g, "") === hash)return gradientIndex = index, !1
            })
        }

        function _loadGradientPallete() {
            gradients.reverse(), $.each(gradients, function (index, item) {
                item.id = index
        ...
        function _updateInfobox() {
            var newGradient = gradients[gradientIndex], contrastColor = $.fn.getContrastingText(newGradient.colors[1]), data = {
                gradient: newGradient,
                contrastColor: contrastColor
            }, gradientInfo = Mustache.render(infoTemplate, data);
            $("#infobox").stop().animate({opacity: .5}, 100, function () {
                $(this).html(gradientInfo).animate({opacity: 1}, {duration: 100}), canvas.trigger("infobox-updated")
            })
        }

        function _updateCodeBox() {
            var newGradient = gradients[gradientIndex], gradientInfo = Mustache.render(codeTemplate, newGradient);
            $("#codebox").stop().animate({opacity: .9}, 100, function () {
                $(this).html(gradientInfo).animate({opacity: 1}, {duration: 100})
            })
        }

        function _renderDisplay(direction) {
            canvas.trigger("updating-gradient"), direction = "undefined" == typeof direction ? "no direction set" : direction, "up" == direction ? (gradientIndex++, gradientIndex = gradientIndex >= gradients.length ? 0 : gradientIndex) : "down" == direction ? (gradientIndex--, gradientIndex = gradientIndex <= -1 ? gradients.length - 1 : gradientIndex) : direction === parseInt(direction) && (gradientIndex = direction), _updateGradient(), _updateInfobox(), _updateCodeBox(), _updateHash()
        }

        function _bootstrap() {
            canvas.on("gradientsLoaded", function () {
                var randomIndex = Math.floor(Math.random() * (gradients.length - 1 - 0 + 1)) + 0;
                gradientIndex = randomIndex, _loadHash(), _renderDisplay(), _loadGradientPallete()
            })
        }
        ...
        function init() {
            _loadGradients(), _bootEventHandlers(), _bootstrap()
        }
        ...
        function getGradients() {
            ...
            }), window.console.table ? console.table(gradientsArray) : console.log(gradientsArray)
        }

        var gradients, gradientIndex, canvas = $("#canvas"), pallete = $("#pallete-list"), infoTemplate = $("#gradient-info-template").html(), codeTemplate = $("#gradient-code-template").html(), palleteTemplate = $("#gradient-pallete-template").html();
        return {init: init, getGradients: getGradients}
    ...
    uiGradients.Infobox = function () {
        ...
        function _bindEvents() {
            color1.on("ready", function (readyEvent) {
                color1.on("aftercopy", function (event) {
                    $(".infobox__color1-success").show(), setTimeout(function () {
                        $(".infobox__color1-success").hide()
                    }, 1e3)
                })
            }), color2.on("ready", function (readyEvent) {
                color2.on("aftercopy", function (event) {
                    $(".infobox__color2-success").show(), setTimeout(function () {
                        $(".infobox__color2-success").hide()
                    }, 1e3)
                })
            })
        }
        ...
        var color1, color2, canvas = $("#canvas");
        return {init: init}
    ...
    uiGradients.Keys = function () {
        function _bindEvents() {
            $(document).on("keydown", function (event) {
                switch (event.which) {
                    case 37:
                        if (ga("send", "event", "keyboard navigation", "left", "prev gradient"), pallete.hasClass("is-active"))return;
                        canvas.trigger("changeDown");
                        break;
                    case 39:
                        if (ga("send", "event", "keyboard navigation", "right", "next gradient"), pallete.hasClass("is-active"))return;
                        canvas.trigger("changeUp");
                        break;
                    case 16:
                        ga("send", "event", "keyboard navigation", "shift", "show pallete"), pallete.trigger("toggle");
                        break;
                    case 27:
                        ga("send", "event", "keyboard navigation", "esc", "hide pallete"), pallete.trigger("close");
                        break;
                    default:
                        return
                }
        ...
        var canvas = $("#canvas"), pallete = $("#pallete");
        return {init: init}
    ...
    uiGradients.Pallete = function () {
        function _show() {
            uiGradients.Popup.reset(), pallete.addClass("is-active")
        }
        ...
        function _bindEvents() {
            palleteTrigger.on("click", function (event) {
                event.stopPropagation(), _show()
            }), $("#pallete-list").on("click", ".pallete__item", function (event) {
                var id = $(this).data();
                canvas.trigger("showGradient", [id]), _close()
            })
        }

        function init() {
            _bindEvents(), _bootEventHandlers()
        }

        var canvas = $("#canvas"), pallete = $("#pallete"), palleteTrigger = $("#do-show-pallete");
        return {init: init}
    }(), window.uiGradients.Pallete.init()
}(window, document, $), window.uiGradients = window.uiGradients || {}, uiGradients.Popup = uiGradients.Popup || {}, function (window, document, $) {
    "use strict";
    uiGradients.Popup.reset = function () {
        $(".popup").removeClass("is-active")
    }, uiGradients.Popup.Github = function () {
        function show() {
            pallete.hasClass("is-active") || popup.addClass("is-active")
        }
            ...
            popupTrigger.on("click", function (event) {
                ...
                switch (event.which) {
                    case 27:
                        ...
                        break;
                    case 32:
                        uiGradients.Popup.reset(), ga("send", "event", "footer-buttons", "keydown", "add new gradient"), show();
                        break;
                    default:
                        return
                }
            }), $(document).on("click", function (event) {
                popup.hasClass("is-active") && (popup.is(event.target) || 0 !== popup.has(event.target).length || close())
            })
        }
        ...
        var pallete = $("#pallete"), popup = $("#popup-github"), popupTrigger = $("#do-show-github-popup");
        return {init: init}
    }(), window.uiGradients.Popup.Github.init(), uiGradients.Popup.Code = function () {
        function show() {
            pallete.hasClass("is-active") || popup.addClass("is-active")
        }
            ...
            popupTrigger.on("click", function (event) {
                ...
                switch (event.which) {
                    case 27:
                        ...
                        break;
                    case 13:
                        uiGradients.Popup.reset(), ga("send", "event", "footer-buttons", "keydown", "copy gradient code"), show();
                        break;
                    default:
                        return
                }
        ...
        var pallete = $("#pallete"), popup = $("#popup--code"), popupTrigger = $("#do-show-code-popup");
        return {init: init}
    ...
    uiGradients.Track = function () {
        ...
        var copyColor1 = $("do-copy-colour1"), copyColor2 = $("do-copy-colour2"), github = $("#github-icon"), twitter = $("#twiiter-link"), shareTwitterMob = $("#twitter-mobicon"), shareTwitter = $("#share-twitter"), shareFacebook = $("#share-facebook"), showAllGradients = $("#do-show-pallete"), addNewGradient = $("#do-show-github-popup"), copyGradientCode = $("#do-show-code-popup"), plugFilter = $("#plug-filter"), plugPodcast = $("#plug-podcast");
        return {init: init}
    }(), window.uiGradients.Track.init()
}(window, document, $), function () {
    for (var method, noop = function () {
    }, methods = ["assert", "clear", "count", "debug", "dir", "dirxml", "error", "exception", "group", "groupCollapsed", "groupEnd", "info", "log", "markTimeline", "profile", "profileEnd", "table", "time", "timeEnd", "timeline", "timelineEnd", "timeStamp", "trace", "warn"], length = methods.length, console = window.console = window.console || {}; length--;)method = methods[length], console[method] || (console[method] = noop)
}(), console.log("====================================================================="), console.log("uigradients.com - Dual tone colour gradients for design and code"), console.log("====================================================================="), console.log("A little project by www.twitter.com/_ighosh");